...
luh_pop_pos <- brm(bf(mu ~ forest.diff_scaled + (1|biome)), 
                   data = mus_luh_pos, 
...
summary(luh_pop_pos)
plot(luh_pop_pos)
save(luh_pop_pos, file = "data/output/luh_pop_pos2018.RData")
...
luh_pop_neg <- brm(bf(mu ~ forest.diff_scaled + (1|biome)), 
                   data = mus_luh_neg, 
                   prior = hier_prior_random, iter = 6000,
                   warmup = 2000,
                   inits = '0',
...
summary(luh_pop_neg)
plot(luh_pop_neg)
save(luh_pop_neg, file = "data/output/luh_pop_neg2018.RData")
...
save(hansen_pop_gain_pos, file = "data/output/hansen_pop_gain_pos2018.RData")
...
hansen_pop_loss <- brm(bf(mu ~ sum_loss_km_scaled + (1|biome)), 
...
save(hansen_pop_loss, file = "data/output/hansen_pop_loss_pos2018.RData")
...
hansen_pop_loss_neg <- brm(bf(mu ~ sum_loss_km_scaled + (1|biome)), 
...
save(hansen_pop_loss_neg, file = "data/output/hansen_pop_loss_neg2018.RData")
...
luh_pop_pos_hab <- brm(bf(mu ~ forest.diff_scaled + habitat + (1|biome)), 
...
save(luh_pop_pos_hab, file = "data/output/luh_pop_pos_hab2018.RData")
...
luh_pop_neg_hab <- brm(bf(mu ~ forest.diff_scaled + habitat + (1|biome)), 
...
save(luh_pop_neg_hab, file = "data/output/luh_pop_neg_hab2018.RData")
...
save(sp_luh_cont_pos, file = "data/output/sp_luh_cont_pos2018.RData")
...
sp_luh_cont_neg <- brm(bf(slope ~ forest.diff_scaled + (1|Biome)), 
                       data = slopes_luh_neg, 
                       prior = hier_prior_random_b, iter = 6000,
                       warmup = 2000,
                       inits = '0',
...
save(sp_luh_cont_neg, file = "data/output/sp_luh_cont_neg2018.RData")
...
slopes_forest <- rbind(slopes_sum2[, 1:8], slopes_sum_Sept2018[, 1:8])

# ** Forest gain - richness gains ----
# 2000 - 2016

# Add the biome data
biomes <- slopes_luh %>% dplyr::select(rarefyID, Biome)
...
save(hansen_sp_gain_pos, file = "data/output/hansen_sp_gain_pos2018.RData")
...
hansen_sp_gain_neg <- brm(bf(slope ~ sum_gain_km_scaled + (1|Biome)), 
...
save(hansen_sp_gain_neg, file = "data/output/hansen_sp_gain_neg2018.RData")
...
hansen_sp_loss_pos <- brm(bf(slope ~ sum_loss_km_scaled + (1|Biome)), 
...
save(hansen_sp_loss_pos, file = "data/output/hansen_sp_loss_pos2018.RData")
...
hansen_sp_loss_neg <- brm(bf(slope ~ sum_loss_km_scaled + (1|Biome)), 
...
save(hansen_sp_loss_neg, file = "data/output/hansen_sp_loss_neg2018.RData")
...
# Add biome data to use as a random effect to account for the spatial
# clustering of time series
slopes_luh_Jtu <- left_join(slopes_luh_Jtu, biomes.bt, by = "rarefyID") %>%
  distinct()
...
prior2b <- c(set_prior(prior = 'normal(0,6)', class='b', coef='forest.diff_scaled'), 	# global slope
             set_prior(prior = 'normal(0,6)', class='Intercept', coef=''), 	# global intercept
...
Jtu_luh_cont <- brm(bf(final_tu ~ forest.diff_scaled + (1|Biome), 
                       coi ~ 1, zoi ~ 1),
...
plot(Jtu_luh_cont)
save(Jtu_luh_cont, file = "data/output/Jtu_luh_cont2018.RData")
...
# Set priors
prior4b <- c(set_prior(prior = 'normal(0,6)', class='b', coef='sum_gain_km_scaled'), 	# global slope
             set_prior(prior = 'normal(0,6)', class='Intercept', coef=''), 	# global intercept
             set_prior("normal(0,.5)", class = "Intercept", dpar = "zoi"),
             set_prior("normal(0,.5)", class = "Intercept", dpar = "coi"), 		
             set_prior(prior = 'cauchy(0,2)', class='sd'))  	# group-level intercepts and slopes
...
prior3b <- c(set_prior(prior = 'normal(0,6)', class='b', coef='sum_loss_km_scaled'), 	# global slope
             set_prior(prior = 'normal(0,6)', class='Intercept', coef=''), 	# global intercept
             set_prior("normal(0,.5)", class = "Intercept", dpar = "zoi"),
             set_prior("normal(0,.5)", class = "Intercept", dpar = "coi"), 		
             set_prior(prior = 'cauchy(0,2)', class='sd'))	# group-level intercepts and slopes
